...
/// [stepState].
typedef StepIconBuilder = Widget? Function(int stepIndex, StepIcon stepState);
...
class MyStepper extends StatefulWidget {
  ...
  ///
  /// When overriding icon for [StepIcon.error], please return
  /// a widget whose width and height are 14 pixels or less to avoid overflow.
  ///
  /// If null, the default icons will be used for respective [StepIcon].
...
class _MyStepperState extends State<MyStepper> with TickerProviderStateMixin {
  ...
  final Map<int, StepIcon> _oldStates = <int, StepIcon>{};
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    final StepIcon state = oldState ? _oldStates[index]! : widget.steps[index].state;
  ...
  Widget _buildCircle(int index, bool oldState) {
    return Container(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index, oldState && widget.steps[index].state == StepIcon.error),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == StepIcon.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != StepIcon.error) {
      ...
      children: <Widget>[
        for (int i = 0; i < widget.steps.length; i += 1)
          Column(
            ...
            children: <Widget>[
              InkWell(
                onTap: widget.steps[i].state != StepIcon.disabled ? () {
                ...
                canRequestFocus: widget.steps[i].state != StepIcon.disabled,
